...
RecipeBook::RecipeBook(QWidget *parent) : QWidget(parent){
    ...
    product_name_line->setReadOnly(true); //установили поле только для чтения, можем только отображать,
    ...
    find_recipe_button->hide(); //кнопка скрыта. будет отображена только после того, как пользователь
    ...
    //TODO: можно ли избавиться от копипаста и вынести это в функцию с параметрами (название кнопки (1), ф-я(4))
    connect(add_product_button, SIGNAL(clicked()), this, SLOT(add_product_func()));
    connect(put_in_basket_button, SIGNAL(clicked()), this, SLOT(put_in_basket_func()));
    connect(find_recipe_button, SIGNAL(clicked()), this, SLOT(find_recipe_func()));
    connect(check_basket_button, SIGNAL(clicked()), this, SLOT(check_basket_func()));
    connect(cancel_button, SIGNAL(clicked()), this, SLOT(cancel_func()));

    connect(next_recipe_button, SIGNAL(clicked()), this, SLOT(next_recipe_func()));
    connect(previous_recipe_button, SIGNAL(clicked()), this, SLOT(previous_recipe_func()));
    ...
    button_layout1->addStretch(); //разместить кнопки ближе к верхней части экрана
...
void RecipeBook::add_product_func(){
    ...
    product_name_line->setFocus(Qt::OtherFocusReason);//установка фокуса ввода на prosuct_name_line
    ...
    //if (!basket.isEmpty())
    cancel_button->show();
    //после добавления нового продукта на экране отображаются кнопки "find recipe" и "to prev product"
}
...
void RecipeBook::find_recipe_func(){
    ...
    //                  tr("empty grocery list from basket"), tr("please add at least one product"));
    //} else
