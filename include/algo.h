...
using std::vector, std::unordered_set, std::tuple, std::string, std::pair,
    std::tuple, search::product, std::set, std::list;

///////////////////////////////////////////

class ingredients_to_recipe {  // сценарий получения рецептов по продуктам
    vector<pair<string, vector<product>>>
        all_recipes;  // вектор рецептов из csv
    vector<product>
        ingredients_of_request;  // ингредиенты, выпадающие на запрос
    list<product> chosen_ingredients;
    list<product> bad_ingredients;  // нежелательные ингредиенты
    vector<tuple<int, string, vector<product>>> recommended_recipes; /* рецепты,
  полученные по продуктам (количество совпавших продуктов, название рецепта,
  список продуктов с их количеством */
