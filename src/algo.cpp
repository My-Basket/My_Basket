...
using std::vector, std::unordered_set, std::tuple, std::string, std::pair,
    std::tuple, std::ifstream;
...
void ingredients_to_recipe::search_ingredient(string &name_of_ingredient) & {
    ...
    file_.close();
    get_prod_top_by_name(name_of_ingredient, size_of_top,
                         ingredients_of_request);
}

[[nodiscard]] vector<product> ingredients_to_recipe::show_res_of_request()
    const & {
    return ingredients_of_request;
}

void ingredients_to_recipe::choose_ingredient_in_menu(
    std::list<product>::const_iterator it) & {
    chosen_ingredients.push_back(*it);
}

void ingredients_to_recipe::remove_ingredient_from_menu(
    std::list<product>::const_iterator it) & {
    chosen_ingredients.erase(it);
}

void ingredients_to_recipe::choose_bad_ingredients(
    std::list<product>::const_iterator it) & {
...
void ingredients_to_recipe::search_recipes() &{
    ...
    for (const auto &recipe: all_recipes) {
        ...
        if (counter > 0 && !is_bad_recipe) {
            recommended_recipes.emplace_back(counter, recipe.first,
recipe.second);
        }
        num_of_recipe++;
    }
    std::sort(recommended_recipes.begin(), recommended_recipes.end(), [](auto
const &t1, auto const &t2) { return std::get<0>(t1) > std::get<0>(t2);
...
[[nodiscard]] vector<pair<string, vector<product>>>
recipe_to_ingredients::get_recipes() const {
